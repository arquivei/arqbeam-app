...
func inferBinaryName() string {
	...
	if strings.Contains(bin, "go-build") || strings.Contains(bin, "__debug__") {
		...
		log.Warn().Msg("[arqbeam] Unable to get current binary name. This is expected if the code is being executed by 'go run'.")
		return ""
	}
	log.Trace().Str("worker_binary", bin).Msg("[arqbeam] Binary name inferred.")
	return bin
}
